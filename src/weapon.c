...
void		touch_by_enemy(t_struct *p)
{
	...
	while (i < 10)
	{
		if (i < 10 && p->k == p->sprite[i].k && (int)p->sprite[i].x == (int)p->c->p_x && (int)p->sprite[i].y == (int)p->c->p_y && p->life >= 0)
		{
			p->life -= 0.2;
		}
		else if (i < 16 && p->k == p->sprite[i].k && (int)p->sprite[i].x == (int)p->c->p_x && (int)p->sprite[i].y == (int)p->c->p_y)
			p->life -= 10;
		i++;
	}
...
void		hit_enemy(t_struct *p)
{
	...
	while (i < 10)
	{
	if (p->keypress[KEY_SPACEBAR] == 1 && ((int)p->sprite[i].x == (int)p->c->p_x && (int)p->sprite[i].y == (int)p->c->p_y))
	{
//		if (p->sprite[i].pv == 10)
//			p->srite[i].k = 6;
		p->sprite[i].pv--;
